...
create_ansible_inventory() {
    ...
    APP_PRIVATE_IP=${APP_PRIVATE_IP:-$(terraform output -raw app_private_ip 2>/dev/null || echo "")}
    JENKINS_PRIVATE_IP=${JENKINS_PRIVATE_IP:-$(terraform output -raw jenkins_private_ip 2>/dev/null || echo "")}
    cd "$PROJECT_ROOT"
    ...
    # Include private IPs so Jenkins can SSH within the VPC
    cat > "$ANSIBLE_DIR/inventory/staging.ini" << EOF
# Auto-generated by bootstrap script
# Generated: $(date -u +"%Y-%m-%dT%H:%M:%SZ")

[jenkins]
jenkins-server ansible_host=$JENKINS_IP private_ip=$JENKINS_PRIVATE_IP

[app]
app-server ansible_host=$APP_IP private_ip=$APP_PRIVATE_IP
...
push_inventory_to_git() {
    log_step "Pushing Updated Inventory to Git"
    
    if [[ "$DRY_RUN" == true ]]; then
        log_info "[DRY-RUN] Would push inventory to Git"
        return
    fi
    
    cd "$PROJECT_ROOT"
    
    # Stage the inventory file first, then check for actual changes
    git add "$ANSIBLE_DIR/inventory/staging.ini"
    
    if git diff --cached --quiet "$ANSIBLE_DIR/inventory/staging.ini" 2>/dev/null; then
        log_info "No inventory changes to push"
        return
    fi
    
    log_info "Committing and pushing updated staging inventory..."
    git commit -m "Update staging inventory with current IPs (auto-generated by bootstrap)" || true
    git push origin main || {
        log_warning "Could not push inventory to Git. Jenkins may use stale IPs."
        log_warning "Manually push with: git push origin main"
    }
    
    log_success "Inventory pushed to Git - Jenkins will use current IPs"
}
